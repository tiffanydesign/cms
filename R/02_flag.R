...
master_long <- master_long %>%
  ...
  mutate(
    # Skipped trial: participant accidentally clicked rest-window "continue"
    # before the video played; paddle_time_s == -1 is the sentinel. Raw
    # TTC_s / Q1–Q4 values are preserved but must be excluded from analyses.
    flag_skipped_trial   = !is.na(paddle_time_s) & paddle_time_s == -1,
    flag_no_response     = is.na(paddle_time_s) | is.na(TTC_s) | flag_skipped_trial,
    flag_missing_rating  = is.na(Q1) | is.na(Q2) | is.na(Q3) | is.na(Q4) | flag_skipped_trial,
    flag_low_eye_quality = !is.na(valid_ratio) & valid_ratio < EYE_QUALITY_THRESHOLD,
    flag_extreme_ttc     = !is.na(TTC_s) & !flag_skipped_trial &
                           (TTC_s < 0 | TTC_s > 19 | TTC_s < lo | TTC_s > hi),
...
flag_cols <- c("flag_skipped_trial", "flag_no_response", "flag_missing_rating",
