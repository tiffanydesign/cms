...
# Exclude skipped trials (flag_skipped_trial) before aggregating so that
# P03/P11 scene-C means at 8.33 Hz/60% are computed from scenes A+B only.
step1 <- master %>%
...
step2 <- master %>%
  filter(!is.na(modulation_depth), !flag_skipped_trial) %>%
...
flicker_trials <- master %>%
  filter(!is.na(modulation_depth), !is.na(TTC_s), !flag_skipped_trial) %>%
  mutate(frequency = factor(as.character(frequency), levels = c("8.33","12.5","25")))
